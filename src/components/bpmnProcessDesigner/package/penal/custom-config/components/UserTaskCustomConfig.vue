 ...
 * @LastEditTime: 2025-12-25 16:01:35
 * @LastEditors: zhangsd
 * @Description: 用户任务自定义配置
 * @FilePath: \view\src\components\bpmnProcessDesigner\package\penal\custom-config\components\UserTaskCustomConfig.vue
-->
<!-- UserTask 自定义配置：
     1. 审批人与提交人为同一人时
     2. 审批人拒绝时
...
<template>
	<div>
		...
		<el-form-item prop="rejectHandlerType">
			<el-radio-group v-model="rejectHandlerType" :disabled="returnTaskList.length === 0" @change="updateRejectHandlerType">
				<div class="flex-col">
...
<script setup>
import { ref, inject, watch, nextTick, toRaw, onMounted } from 'vue'
import {
...
const btnDisplayNameBlurEvent = index => {
...
const resetCustomConfigList = () => {
	...
	elExtensionElements.value =
		bpmnElement.value.businessObject?.extensionElements ?? bpmnInstances().moddle.create('bpmn:ExtensionElements', { values: [] })
	...
	if (formType.value === BpmModelFormType.NORMAL) {
		const fieldsPermissionList = elExtensionElements.value.values?.filter(ex => ex.$type === `${prefix}:FieldsPermission`)
...
function useButtonsSetting() {
	const buttonsSetting = ref()
	// 操作按钮显示名称可编辑
	const btnDisplayNameEdit = ref([])
	const changeBtnDisplayName = index => {
...
const updatePermission = type => {
	fieldsPermissionEl.value.forEach(field => {
		field.permission =
			type === 'READ' ? FieldPermissionType.READ : type === 'WRITE' ? FieldPermissionType.WRITE : FieldPermissionType.NONE
	})
}
