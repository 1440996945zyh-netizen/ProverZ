 ...
 * @LastEditTime: 2025-12-19 11:14:09
 * @LastEditors: zhangsd
 * @Description: 时间事件配置
 * @FilePath: \view\src\components\bpmnProcessDesigner\package\penal\time-event-config\TimeEventConfig.vue
-->
...
<template>
	<div class="panel-tab__content">
		...
		<div style="margin-top: 10px; display: flex; align-items: center">
			...
			>
				<template #suffix>
					...
					<el-button v-if="type === 'time'" @click="showDatePicker = true" style="margin-left: 4px" circle size="small">
						<el-icon><Calendar /></el-icon>
					</el-button>
					<el-button v-if="type === 'duration'" @click="showDurationDialog = true" style="margin-left: 4px" circle size="small">
						<el-icon><Timer /></el-icon>
					</el-button>
					<el-button v-if="type === 'cycle'" @click="showCycleDialog = true" style="margin-left: 4px" circle size="small">
		...
		<!-- 时间选择器 -->
		<Dialog v-model:visible="showDatePicker" title="选择时间" width="400px" >
			<el-date-picker v-model="dateValue" type="datetime" placeholder="选择日期时间" style="width: 100%" @change="onDateChange" />
			<template #footer>
				<el-button @click="handleDialogCancel('date')">取消</el-button>
				<el-button type="primary" @click="onDateConfirm">确定</el-button>
			</template>
		</Dialog>
		<!-- 持续时长选择器 -->
		<Dialog v-model:visible="showDurationDialog" title="时间配置" width="600px">
			<DurationConfig :value="condition" @change="onDurationChange" />
			<template #footer>
				<el-button @click="handleDialogCancel('duration')">取消</el-button>
				<el-button type="primary" @click="onDurationConfirm">确定</el-button>
			</template>
		</Dialog>
		<!-- 循环配置器 -->
		<Dialog v-model:visible="showCycleDialog" title="时间配置" width="800px" >
			<CycleConfig :value="condition" @change="onCycleChange" />
			<template #footer>
				<el-button @click="handleDialogCancel('cycle')">取消</el-button>
				<el-button type="primary" @click="onCycleConfirm">确定</el-button>
			</template>
		</Dialog>
		<!-- 帮助说明 -->
		<Dialog v-model:visible="showHelp" title="格式说明" width="600px" >
			<div v-html="helpHtml"></div>
			<template #footer>
				<el-button @click="handleDialogCancel('help')">关闭</el-button>
			</template>
		</Dialog>
	</div>
</template>
...
import { CircleCheckFilled, WarningFilled, QuestionFilled, Calendar, Timer, Setting } from '@element-plus/icons-vue'
import DurationConfig from './DurationConfig.vue'
import CycleConfig from './CycleConfig.vue'
import Dialog from "@/components/Dialog"

// 移除TS类型断言，简化bpmnInstances获取
...
// 新增：统一的弹窗取消处理方法（核心优化）
const handleDialogCancel = (dialogType) => {
  // 标记正在关闭弹窗
  isClosingDialog.value = true
  // 根据类型关闭对应弹窗
  if (dialogType === 'date') showDatePicker.value = false
  if (dialogType === 'duration') showDurationDialog.value = false
  if (dialogType === 'cycle') showCycleDialog.value = false
  if (dialogType === 'help') showHelp.value = false
  // 延迟清除标记（确保输入框焦点事件触发时标记仍有效）
  setTimeout(() => {
...
const onDurationConfirm = () => {
  // 调用统一取消方法（复用标记逻辑）
  handleDialogCancel('duration')
  updateNode()
}
...
const onCycleConfirm = () => {
  // 调用统一取消方法（复用标记逻辑）
  handleDialogCancel('cycle')
  updateNode()
}

// 优化focus事件：判断是否正在关闭弹窗，若是则不执行打开逻辑（核心优化）
