		...
		<el-dialog :title="dialogTitle || '审批记录'" v-model="dialogVisible" width="1000px">
			<el-row>
				<el-table :data="selectTasks" size="small" border header-cell-class-name="table-header-gray">
					...
					<el-table-column label="发起人" prop="assigneeUser.userName" min-width="100" align="center" v-else />
		...
		<div style="position: absolute; top: 0; left: 0; width: 100%">
			<el-row type="flex" justify="end">
				<el-button-group key="scale-control" size="default">
					<el-button size="default" :plain="true" :disabled="defaultZoom <= 0.3" icon="ZoomOut" @click="processZoomOut()" />
					<el-button size="default" style="width: 90px">
						{{ Math.floor(defaultZoom * 10 * 10) + '%' }}
					</el-button>
					<el-button size="default" :plain="true" :disabled="defaultZoom >= 3.9" icon="ZoomIn" @click="processZoomIn()" />
...
const selectActivityType = ref(undefined) // 选中 Task 的活动编号
...
const onSelectElement = element => {
	...
	if (activityType === 'bpmn:UserTask') {
		...
		selectTasks.value = tasks.value.filter(item => item?.taskDefinitionKey === element.id)
