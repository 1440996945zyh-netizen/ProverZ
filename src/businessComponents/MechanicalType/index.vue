		...
		>
			...
			<vxe-column field="num" title="编号/数量" :edit-render="{ autofocus: '.el-input__inner' }">
				<template #default="{ row }">
					<span v-if="row.equipmentTypeName !== '门座式起重机'">{{ row.num }}</span>
					<span v-else>{{ format(row) }}</span>
				</template>
				<template #edit="{ row }">
					<el-input
						v-if="row.equipmentTypeName !== '门座式起重机'"
						v-model="row.num"
						type="text"
						@input="row.num = proxy.verify('positiveNumber', row.num)"
					></el-input>
					<Select v-else :selectData="equipData" multiple v-model:value="row.equipmentIds" v-model:label="row.equipmentNames" />
				</template>
			</vxe-column>
		</vxe-table>
		<div class="button">
