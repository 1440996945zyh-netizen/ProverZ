...
import { CandidateStrategy, NodeId, FieldPermissionType } from '@/components/bpmnProcessDesigner/package/penal/consts'
...
const submitForm = async () => {
	...
	try {
		...
		router.push({ name: 'BpmProcessInstance' })
		// await push({
...
/* 表单滚动区域 */
.form-scroll-area {
	height: calc(100vh - var(--top-tool-height) - var(--tags-view-height) - var(--app-footer-height) - 35px - 105px - 40px);
	max-height: calc(100vh - var(--top-tool-height) - var(--tags-view-height) - var(--app-footer-height) - 35px - 105px - 40px);
	overflow: auto;
}
