<template>
  <div>
    ...
    <el-dialog type="primary" v-model="dialogVisible" title="港存导入" width="35%" :before-close="handleClose">
      <el-upload
        ref="upload"
        action="#"
      ...
      >
        <!--accept 只接受某种格式的文件-->
        <el-button type="primary" style="margin-bottom: 20px">导入港存</el-button>
...
import api from '@/api/largescreen/sPortStorage/index.js'
...
const handleDelete = row => {
  ...
  proxy.$modal
    .confirm('是否确认删除货名为" ' + deleteRow.cargoName + ' "的数据项?').then(function () {
