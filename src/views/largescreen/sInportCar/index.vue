<template>
  <div>
    ...
    <el-dialog type="primary" v-model="dialogVisible" title="在港车辆导入" width="35%" :before-close="handleClose">
      <el-upload
        ref="upload"
        action="#"
      ...
      >
        <!--accept 只接受某种格式的文件-->
        <el-button type="primary" style="margin-bottom: 20px">导入在港车辆</el-button>
...
import api from '@/api/largescreen/sInportCar/index.js'
...
const tableColumns = ref([
  ...
  {label: '计划量', prop: 'planTon'},
  {label: '货主', prop: 'cargoOwner'},
  {label: '进港时间', prop: 'inPortDate'},
  {label: '在港时长', prop: 'inPortTime'},
...
const handleDelete = row => {
  ...
  proxy.$modal
    .confirm('是否确认删除车牌号为" ' + deleteRow.carNum + ' "的数据项?').then(function () {
