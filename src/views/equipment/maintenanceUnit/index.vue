...
const getStatistics = () => {
	api.getOutTypeNum().then(res => {
		if (res.code == '0000') {
			statistics.value.total = res.data.unitNameSum || 0
			statistics.value.internal = res.data.inSunm || 0
			statistics.value.external = res.data.outSum || 0
			let arr = [
				{
