...
import api from '@/api/equipment/maintainPlan/index'
...
const tableColumns = reactive([
  ...
  { label: '设备小类', prop: 'equipSmallCategoryName', },
	...
	{ label: '周期', prop: 'cycle', align: 'right' },
	{ label: '时限（天）', prop: 'timeLimit', align: 'right', width:140 },
	{ label: '保养人员', prop: 'inspectorName' },
	{ 
    label: '提报状态', 
    prop: 'status',
		align: 'center',
			...
			return [
				h(
					ElButton,
					{
						...
						link: true,
						icon: 'Position',
						permission: 'equipment:maintainPlan:report',
					},
					{
						default: () => '提报',
					}
				...
				h(
					ElButton,
					{
						...
						link: true,
						icon: 'Edit',
						permission: 'equipment:maintainPlan:update',
					},
					{
						default: () => '编辑',
					}
				...
				h(
					ElButton,
					{
						...
						link: true,
						icon: 'Delete',
						permission: 'equipment:maintainPlan:delete',
					},
					{
						default: () => '删除',
					}
    ...
    if (params.setDate) {
      ...
      params.setDate = params.setDate.join(",")
		...
		if (!viewRef.value.tableData || viewRef.value.tableData.length==0) {
			return proxy.$message.warning('请选择润滑保养标准')
		}
...
const handleReport = row => {
	proxy.$modal.confirm("确认提报？").then(res => {
    api.report({id: row.id}).then(res => {
      proxy.$message.success(res.msg)
			getList(queryParams.value)
    })
  })
