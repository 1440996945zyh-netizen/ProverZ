 ...
 * @LastEditTime: 2025-09-03 17:43:21
 * @LastEditors: zhangsd
 * @Description: 通用工具类使用场景 common.js
      ...
      <el-row :gutter="20" class="mt-4">
        ...
        <el-col :xs="24" :md="12">
          <el-form-item label="原始数字（金额）" label-width="140px">
            <el-input
              v-model="rawMoney"
              placeholder="示例：123456.78 / -98765.43 / 100000"
              clearable
              type="number"
            ></el-input>
          ...
          <el-form-item label="格式化结果" label-width="140px">
            <el-input
              ...
              readonly
              placeholder="示例：123,456.78 / -98,765.43 / 100,000.00"
            ></el-input>
          </el-form-item>
        </el-col>
        ...
        <el-col :xs="24" :md="12">
          ...
          <el-form-item label="反格式化结果（数字）" label-width="140px">
            <el-input
              type="textarea"
              :autosize="{ minRows: 2, maxRows: 3 }"
      ...
      <div class="space-y-4 mt-4">
        ...
        <el-form-item label="添加日期后参数" label-width="120px">
          <el-input
            type="textarea"
            :autosize="{ minRows: 3, maxRows: 5 }"
            v-model="paramsWithDateRangeStr"
            readonly
          ></el-input>
        </el-form-item>
      </div>
    ...
    <el-card class="mb-8 shadow-md" border>
      ...
      <div class="space-y-4 mt-4">
        <el-form-item label="输入路径" label-width="100px">
          <el-input
            v-model="pathInput"
            placeholder="输入需要规范化的路径（如：/user//profile//settings/）"
            clearable
          ></el-input>
        </el-form-item>
        <el-form-item label-width="100px">
...
import {
  ...
  ElButton, ElSpace, ElTag, ElTree, ElDatePicker, ElSelect, ElOption, ElTable, ElTableColumn
} from 'element-plus'
...
// -------------------------- 新增：9. 金额格式化/反格式化演示 --------------------------
const rawMoney = ref('123456.78') // 原始金额数字（输入）
const formattedMoneyResult = ref('') // 格式化后的金额字符串（输出）
const formattedMoneyInput = ref('123,456.78') // 格式化的金额字符串（输入）
const unformattedMoneyResult = ref('') // 反格式化后的金额数字（输出）
...
const handleFormatMoney = () => {
  try {
    ...
    // 调用formatMoney方法，传入数字类型（避免字符串处理异常）
    const result = formatMoney(Number(rawMoney.value))
    formattedMoneyResult.value = `原始值：${rawMoney.value}\n格式化后：${result}`
  } catch (error) {
...
const handleUnformatMoney = () => {
  try {
    ...
    unformattedMoneyResult.value = `原始字符串：${formattedMoneyInput.value}\n反格式化后：${result}\n数据类型：${typeof result}`
  } catch (error) {
