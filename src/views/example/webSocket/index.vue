	...
	<div class="ws-demo-container">
		...
		<div class="header">
			...
			<div class="connection-status">
				...
				<span class="connection-time" v-if="wsStore.connected">连接时间: {{ formatTime(wsStore.connectTimestamp) }}</span>
			</div>
		</div>
			...
			<div class="message-list">
				<div class="message-list-header">
					...
					<div class="header-controls">
						<button class="filter-btn" @click="toggleMessageFilter">
							<i class="icon-filter"></i>
							{{ showOnlyUnread ? '显示全部' : '仅看未读' }}
						</button>
						<button class="clear-btn" @click="wsStore.clearAllMessages" :disabled="filteredMessages.length === 0">
							清空记录
						</button>
					</div>
				</div>
...
const sendCustomMsg = type => {
	...
	console.log('wsStore.mesConnectPerson =>', wsStore.mesConnectPerson)
	// 发送消息
