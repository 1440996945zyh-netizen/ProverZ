<template>
	...
	>
      ...
      <el-collapse-item title="优惠信息" name="2">
          ...
          <el-col :span="6"  v-if="formData.reduceType == '3'">
						<el-form-item label="计费截止时间" prop="calEndDate">
              <el-date-picker
                v-model="formData.calEndDate"
                value-format="YYYY-MM-DD"
                format="YYYY-MM-DD"
                type="date"
                placeholder="计费截止时间"
  ...
  await api.getReduceList(formData.value.cargoInfoId).then(res=>{
      ...
      if(tmpList!==undefined&&tmpList!=null&&tmpList.length>0){
        if(tmpList[0].reduceType=='' || tmpList[0].reduceType==null){
          ...
          publicApi.getBusFiles({businessId:formData.value.cargoInfoId,businessType:"CARGO_REDUCE_PROOF"}).then(res=>{
...
const changeReduceFile = files => {
  ...
  // console.log(formData.value.reduceFileList)
  filesIdList.value = files
