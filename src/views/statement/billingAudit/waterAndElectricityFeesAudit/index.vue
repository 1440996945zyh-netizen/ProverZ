...
<script setup name="waterAndElectricityFeesAudit">
...
const buttonList = reactive([
	{
		...
		click: () => review, // 回调函数
		permission: 'statement:billingAudit:waterAndElectricityFeesAudit:audit', // 权限
	},
	{
		label: '销审', // 按钮名称
		type: 'primary', // 按钮类型
		...
		permission: 'statement:billingAudit:waterAndElectricityFeesAudit:quash', // 权限
	},
])
...
const tableColumns = ref([
  ...
  { label: '作业公司', prop: 'companyName', width: 190 },
	{ label: '客户', prop: 'customerName', width: 190 },
	{ label: '作业过程', prop: 'processName', width: 160 },
	{ label: '船名航次', prop: 'shipNameVoyage', width: 160 },
  { label: '泊位', prop: 'berthName', width: 90 ,    align: 'left'},
  { label: '船代', prop: 'shipCustomerName', width: 180 },
  { label: '作业区域', prop: 'workAreaCd',  width: 100 },
	{ label: '泊位', prop: 'berthName', width: 160 },
	{ label: '贸别', prop: 'tradeType', width: 120 },
	...
	{ label: '结算时间', prop: 'createTime', width: 'time' },
	{
		label: '作业状态',
		prop: 'statusLabel',
		align: 'center',
    ...
    if (waterElectricityList.length) {
      filterArr = rateList.filter(items => {
        ...
        let obj = {
          ...
          taxRate: v1.taxRate,
          amount: (math.numMulti(waterElectricityList.map(v2=>v2.quantity).reduce((sum, x) => math.numAdd(sum, x)), v1.rate)).toFixed(2),
        ...
        obj.taxAmount = (math.numDiv(math.numMulti(obj.amount, math.numDiv(obj.taxRate, 100)),
          math.numAdd(1, math.numDiv(obj.taxRate, 100)))).toFixed(2)
        return obj
      })
    }
...
/*    proxy.$bus.emit('setInitSearchData', { status: '50' })
    proxy.$bus.emit('setInitSearchData', { statementStatus: '10' })*/
    BaseTableRef.value.SearchHeaderRef.searchData.statementStatus = '10'
