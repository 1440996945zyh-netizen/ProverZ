				...
				<el-row :gutter="20">
					<el-col :span="6">
						<el-form-item label="合同" prop="contractId">
							<div style="width: 100%">
								<Select
									:dataConfig="{ url: `/api/external/storageSettle/listContract?handoverlistId=${formData.id?formData.id:0}&date=${formData.date===undefined||formData.date===null?'':formData.date}` }"
          ...
          <el-col :span="6">
            <el-form-item label="是否使用优惠" prop="isUseReduce">
              <Select :selectData="reduceIsList"
                      v-model:value="formData.isUseReduce" @change="isUserReduceChange" :disabled='disableContract'/>
            </el-form-item>
          </el-col>
          <el-col :span="6">
...
const isUserReduceChange = e =>{
  ...
  if(e.value=='1'){
    api.getReduceList(formData.value.cargoInfoId).then(res=>{
    ...
    formData.value.reduceTypeLabel =''
    formData.value.reduceType = -1
  ...
  if(formData.value.contractId!=''){
    //重新计获取堆存费信息
    let params= {
      handoverlistId:formData.value.id,
