...
/*	{
		...
		click: () => review, // 回调函数
    permission:"business:costStatement:auditCost" , // 权限
	},
	{
		label: '销审', // 按钮名称
		type: 'primary', // 按钮类型
    ...
    permission:"business:costStatement:revokeReview" , // 权限
	},*//*{
    label: "上传回执单", // 按钮名称
    type: 'primary', // 按钮类型
    disabled: true,
    ...
    permission:(routeType == '3') ?  'business:costStatement:uploadBusinessFile':'', // 权限
    permission:"business:costStatement:cost" , // 权限
  },
	{
		label: (routeType == '3')?"回执确认":'商务确认', // 按钮名称
		type: 'primary', // 按钮类型
		disabled: true,
		icon: 'View', // 按钮图标，支持element-Plus中所有图标
		click: () => businessAffirm, // 回调函数
		permission:(routeType == '3') ?  'business:costStatement:businessAffirm':'', // 权限
    permission:"business:costStatement:cost" , // 权限
	},
	{
		label: (routeType == '3')?"取消回执确认":'取消商务确认', // 按钮名称
		type: 'primary', // 按钮类型
		disabled: true,
		icon: 'Refresh', // 按钮图标，支持element-Plus中所有图标
		click: () => cancelBusinessAffirm, // 回调函数
		permission: (routeType == '3') ?  'business:costStatement:cancelBusiness':'', // 权限
    permission:"business:costStatement:cost" , // 权限
	},*/
])
...
const dbClickEvent = async () =>{
  ...
  const { data } = await api.listDetailForXCSG(clickRow.value.id)
        ...
        // if(routeType ==  '1' &&  clickRow.value.status == '10'){
          ...
          if( drawerRef.value.formData.settlementBasisName ===undefined || drawerRef.value.formData.settlementBasisName == null || drawerRef.value.formData.settlementBasisName == ''){
            if(clickRow.value.tradeTypeLabel === '内贸'){
              drawerRef.value.formData.settlementBasisName = tmpList[0].settlementBasisName
              drawerRef.value.formData.settlementBasisCode = tmpList[0].settlementBasisCode
            }else if(clickRow.value.tradeTypeLabel === '外贸'){
              drawerRef.value.formData.settlementBasisName = tmpList[0].outerSettlementBasisName
              drawerRef.value.formData.settlementBasisCode = tmpList[0].outerSettlementBasisCode
            }
          }
        }
...
const total = ref(0)
const routeTyoeIs2 = routeType == '2'
// 表格数据
const tableData = ref([])
const tableColumns = ref([
  { label: '序号', type: 'seq', width: 50,align: 'center' },
  { label: 'SCN', prop: 'scn', width: 100 ,isHidden: isHiddenLJLS},
	{ label: '船名航次', prop: 'shipNameVoyage', width: 170 ,isHidden: isHiddenLJLS},
  { label:'票货号',prop:'trustNo',width: 120,isHidden:isHiddenLJLS.value == true ? false:true  },
	{ label: '货主', prop: 'cargoOwnerName', width: 220 },
	{ label: '结算人', prop: 'createByName', width: 100 },
	{ label: '件杂', prop: 'workTypeLabel',  width: 80,align: 'center' },
	...
	{ label: '装卸', prop: 'loadUnload', width: 90,align:'center' },
  { label: '进出口', prop: 'impExpLabel', width: 90,align:'center' },
  ...
  },
  {
    ...
    prop: '是否填写交接清单',
    align: 'center',
    isHidden: isHiddenLJLS.value == true ? false:true ,
    width: 160,
    fixed: 'right',
    render: row => {
      return [
...
const costDetail =  () => {
  ...
  nextTick(() => {
    ...
    console.log(countTableClickRow,"countTableClickRowcountTableClickRowcountTableClickRow")
...
const init = async () => {
  await nextTick(() => {
/*    proxy.$bus.emit('setInitSearchData', { statementStatus: '-1' })
    queryParams.value.statementStatus = '-1'*/
    handoverTableRef.value.SearchHeaderRef.searchData.statementStatus = '-1'
    queryParams.value.statementStatus = '-1'
	})
  //刷新表单数据
	getList(queryParams.value)
