...
<template>
  <div>
    <el-form :model="billingForm" ref="formRef" label-width="110px" :rules="rules" style="margin: 20px 20px">
      ...
      <el-form-item label="申请开票日期" prop="billDate">
        <el-date-picker
          v-model="billingForm.billDate"
          value-format="YYYY-MM-DD"
          format="YYYY-MM-DD"
      ...
      <el-form-item label="计费金额"  v-if="routeType == 1" >
        <el-input v-model="billingForm.amountMoney" type="number"  disabled/>
      </el-form-item>
...
<script setup name="editTrustOrder">
  ...
  const rules = ref({
    ...
    rateId: [{required: true, message: '费率不可为空'}],
  });
  ...
  const changeRate = rateObj =>{
    ...
    if(billingForm.value.billQuantity!==undefined && billingForm.value.billQuantity!==null && billingForm.value.billQuantity!==''){
      ...
      if(processNameList.value.length>0) {
        billingForm.value.processCode = res.data[0].processCode ? res.data[0].processCode : ''
        billingForm.value.processName = res.data[0].processName ? res.data[0].processName : ''
      }
    });
  }
