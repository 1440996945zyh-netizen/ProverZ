...
import api from '@/api/master/tug/index'
...
const tableColumns = ref([
  { label: '拖轮编号', prop: 'tugCode' },
  { label: '拖轮名称', prop: 'tugName' },
  { label: '创建者', prop: 'createByName' },
  { label: '创建时间', prop: 'createTime' },
  { label: '更新者', prop: 'updateByName' },
...
const selectData = reactive([
  {
    name: '拖轮编号', // 搜索框code
    type: 'input', // 搜索框类型
    modelValue: 'tugCode', // 绑定字段
    span: 12, // 占位，共24
  },
  {
    name: '拖轮名称', // 搜索框name
    type: 'input', // 搜索框类型
    modelValue: 'tugName', // 绑定字段
    span: 12, // 占位，共24
  },
])
  ...
  nextTick(() => {
    ...
    api.getDataById(editRow.id).then(res => {
      if (res.code == '0000') {
        ...
        detailRef.value.formData.tugCode = msg.tugCode
        detailRef.value.formData.tugName = msg.tugName
      } else {
