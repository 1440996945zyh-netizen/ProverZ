<template>
  <div class="formData" style="padding: 0px 0px">
    <SearchHeader ref="SearchHeaderRef" class="tab_header" :showNum="4" :selectData="selectData" @searchClick="getList" :buttonList="buttonList" />

    <el-table ref="tableRef" :data="tableData"
              virtual-scroll
              :span-method="bottomSpanMethod"
              border  scrollbar-always-on style="width: 100%;height: 700px;margin-left: 1%" size="large" >
...
import waiFupLjZxExApi from '@/api/produce/waiFuLjZxEx/index.js'
...
const buttonList = reactive([
  {
    label: '审核', // 按钮名称
    type: 'primary', // 按钮类型
    click: () => waiFuLjEx, // 回调函数
    permission: 'produce:waiFuPrdEx:prdEx', // 权限

  }, {
    label: '销审', // 按钮名称
    type: 'primary', // 按钮类型
    click: () => waiFuLjExV, // 回调函数
  ...
  },
  {
    ...
    permission: 'produce:waiFuLjEx:exportExcel', // 权限
...
const waiFuLjEx= async ()=>{
  ...
  await waiFupLjZxExApi.waiFuLjEx(queryParams.value).then(res => {
...
const waiFuLjExV= async ()=>{
  ...
  await waiFupLjZxExApi.waiFuLjExV(queryParams.value).then(res => {
    tableData.value = res.data
    total.value = res.data.totalNum
    loading.value = false
  })
