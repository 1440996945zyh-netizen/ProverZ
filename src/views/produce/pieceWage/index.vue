...
<script setup name="pieceWage">
// import Detail from './detail/index.vue'
import BaseTable from '@/components/BaseTable/index.vue'
import {getCurrentInstance, h, nextTick, reactive, ref, watch} from 'vue'
import { ElButton, ElTag } from 'element-plus'
import api from '@/api/produce/pieceWage/index.js'
...
const salaryQuantity = ref(0) // 数据总数
const salaryTon = ref(0) // 数据总数
const salaryAmount = ref(0) // 数据总数
...
const initData = () => {
	...
	publicApi.getDictList({ types: 'SALARY_STATUS' }).then(res => {
		res.data.SALARY_STATUS.forEach(item => {
			if (item.value != '10') {
		...
		nextTick(() => {
			...
			// 	month: date,
			// })
      //proxy.setEditTableOptions(selectData, { salaryStatusCode: "20" })
      //baseTable.value.SearchHeaderRef.searchData.salaryStatusCode = '20'
			proxy.$bus.emit('setInitSearchData', {
				salaryStatusCode: '30',
			})
		})
...
const selectData = reactive([

	{
		...
		span: 8,
		dataConfig: {
			params: { type: 'DEPT_LEVEL_SALARY', deptLevel: '2' },
			method: 'get',
		},
	},
  {
	...
	},
  {
	...
	/*{
		...
		span: 3,
		dataConfig: { params: { type: 'SUB_WORK_PROCESS' } },
	},*/
	{
		name: '审核状态',
		type: 'select',
...
watch(() =>router.currentRoute.value.path,(newValue,oldValue)=> {
  ...
  if(JSON.stringify(routerQuery) != "{}"){
    isRouter.value = true;
    nextTick(()=>{
      baseTable.value.SearchHeaderRef.searchData.deptId = routerQuery.deptId;
      baseTable.value.SearchHeaderRef.searchData.deptName = routerQuery.deptName;
...
const getList = async e => {
  ...
  queryParams.value.endDate = queryParams.value.date ? queryParams.value.date[1] : ''
	await api.getListSalary(queryParams.value).then(res => {
		tableData.value = res.data.pages
		total.value = res.data.totalNum
	})
  await api.getSalarySum(queryParams.value).then(res=>{
    let data = res.data
    salaryQuantity.value=data.quantity
    salaryTon.value=data.ton
    salaryAmount.value=data.amount
    collectHtml.value = `<p > 总吨数：${salaryTon.value} 总金额：${salaryAmount.value}</p>`
  })
}
const buttonList = reactive([
	...
	},*/
	{
		...
		click: () => pass, // 回调函数
		permission: 'master:produceHrAudit:pass', // 权限
	},
	{
		label: '撤销', // 按钮名称
		type: 'danger', // 按钮类型
		...
		click: () => quash, // 回调函数
		permission: 'master:produceHrAudit:quash', // 权限
	},
  {
    label: '导出', // 按钮名称
    type: 'primary', // 按钮类型
    ...
    permission: 'master:produceHrAudit:export', // 权限
  },
])
// const add = () => {
// 	title.value = '新增'
...
const pass = () => {
  ...
  proxy.$modal.confirm('确定审核' + queryParams.value.startDate + '到' + (queryParams.value.endDate==undefined?getNowDate('YYYY-MM-DD'):queryParams.value.endDate) + '的数据?').then(() => {
		api.examine({
      startDate: auditItem.value.startDate,
      endDate: auditItem.value.endDate,
			flag: auditItem.value.flag,
			isHr: auditItem.value.isHr,
      classCode:auditItem.value.classCode,
      deptId:auditItem.value.deptId,
      companyId:auditItem.value.companyId,
      salaryTypeCode:auditItem.value.salaryTypeCode,
		}).then(res => {
			if (res.code === '0000') {
