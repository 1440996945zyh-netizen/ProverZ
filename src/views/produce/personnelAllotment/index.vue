...
<script setup name="personnelAllotment">
...
import personnelAllotment from "@/api/produce/personnelAllotment/index.js";
...
const selectData = reactive([
	...
	},
  {
    ...
    modelValue: 'isTicket',
    span: 3,
    selectData: [{value: '未分配',label: '未分配'},{value: '人员分配',label: '人员分配'}],
  },
  {
    name: '船名',
    type: 'input',
...
const buttonList = reactive([
	{
		label: '分配', // 按钮名称
		type: 'primary', // 按钮类型
		click: () => personnelAllotmentTicket, // 回调函数
		permission: 'produce:dispatchSecondary:personAllot', // 权限
		disabled: true,
	},
	{
		label: '撤销', // 按钮名称
		...
		permission: 'produce:dispatchSecondary:personAllotRepeal', // 权限
		// disabled: true,
	},
])
...
// 人员分配
const personnelAllotmentTicket = () => {
	getJurisdiction(3)
}
// 获取权限进行编辑回显
...
const harborableColumns = ref([
  ...
  //{ prop: 'ticketTypeLabel', label: '签票类型', width: 120, align: 'left' },
  { prop: 'trustNo', label: '通知单编号', width: 120, align: 'left' },
  { prop: 'cargoInfoNo', label: '票货号', width: 250, align: 'left' },
  { prop: 'companyName', label: '作业公司', width: 250, align: 'left' },
  {
...
const init = async () => {
	await api.getNowDate().then(res => {
		nextTick(() => {
			// proxy.$bus.emit('setInitSearchData', { workDate: res.data.workDate, classCode: res.data.classCode,isTicket:'未分配',laborStatus:'1'})
