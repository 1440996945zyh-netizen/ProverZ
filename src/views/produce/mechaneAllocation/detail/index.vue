  ...
  if(labe=='吨数'){
    ...
    if(groupTableList.value.length>0) {
      groupTableList.value.forEach(item => {
        if (map.has(item.processDetailCode + "_" + item.workPositionCode)) {
          map.set(item.processDetailCode + "_" + item.workPositionCode,map.get(item.processDetailCode + "_" + item.workPositionCode).concat(item.ton))
        } else {
          map.set(item.processDetailCode + "_" + item.workPositionCode, new Array(item.ton))
        }
      })
      groupTableList.value.forEach(item=>{
        if(map.has(item.processDetailCode + "_" + item.workPositionCode)) {
          item.tmpTon = map.get(item.processDetailCode + "_" + item.workPositionCode).reduce( (acc,cur)=>{
            return Math.round((Number(acc)+Number(cur))*1000)/1000;
          },0)
        }
      })
    }
    console.log("吨数失焦事件结束")
  }
}
