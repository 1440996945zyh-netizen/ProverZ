...
const tableColumns = ref([
  ...
  { label: '完工时间', prop: 'workEndTime',width: 160 },
  {
    label: '是否调度确认',
    fixed: 'right',
    prop: 'shipPersonExamineBy',
    align: 'center',
    width: 130,
    render: row => {
      return [
        h(
          ElTag,
          {
            type: row.shipPersonExamineBy != null ? 'primary' : 'info',
          },
          {
            default: () => (row.shipPersonExamineBy != null ? '调度确认' : '未调度确认 '),
          }
        ),
      ]
    },
    ...
    render: row => {
      return [
        h(
          ElTag,
          {
            type: row.shipMacExamineBy != null ? 'primary' : 'info',
          },
          {
            default: () => (row.shipMacExamineBy != null ? '库场确认' : '未库场确认 '),
          }
        ),
      ]
    },
  },
	{
		...
		render: row => {
			return [
				h(
					...
					{
						onClick: () => {
              personClearConfirm(row)
						},
						type: 'primary',
						link: true,
						icon: 'Edit',
            permission: 'produce:shipAdjustmentCompleted:personClearConfirm',
					},
					{
						default: () => '调度确认',
					}
        ...
        h(
          ElButton,
          {
            ...
            link: true,
            icon: 'Edit',
            permission: 'produce:shipAdjustmentCompleted:macClearConfirm',
          },
          {
            default: () => '库场确认',
          }
...
const buttonList = reactive([
  {
    ...
    permission: 'produce:shipAdjustmentCompleted:personnelRevocation', // 权限
  },
  {
    label: '库场撤销', // 按钮名称
    type: 'primary', // 按钮类型
    click: () => macRevocation, // 回调函数
    permission: 'produce:shipAdjustmentCompleted:macRevocation', // 权限
  },
])
...
const personnelRevocation=()=>{
  if (!clickRow.value.shipVoyageItemId) {
    return proxy.$modal.msgError('请选择一条数据')
  }
  if (!clickRow.value.shipPersonExamineBy) {
    return proxy.$modal.msgError('未审核，无需撤销')
  }
  proxy.$modal.confirm('是否调度撤销？').then(() => {
    api.updateShipAdjustClearPersonStatus(clickRow.value.shipVoyageItemId).then(res => {
...
const macRevocation=()=>{
  ...
  if (!clickRow.value.shipMacExamineBy) {
    return proxy.$modal.msgError('未审核，无需撤销')
  }
  proxy.$modal.confirm('是否库场撤销？').then(() => {
    api.updateShipAdjustClearMacStatus(clickRow.value.shipVoyageItemId).then(res => {
      getList(queryParams.value)
      proxy.$modal.msgSuccess('撤销成功')
    })
  })
...
const personClearConfirm = row => {
  const clearConfirmRow = row || clickRow.value
  proxy.$modal
    .confirm('确定调度完工？')
    .then(function () {
      return api.personClearConfirm({shipvoyageItemId:clearConfirmRow.shipVoyageItemId})
    })
    .then(res => {
      getList(queryParams.value)
...
const macClearConfirm = row => {
  const clearConfirmRow = row || clickRow.value
  proxy.$modal
    .confirm('确定库场完工？')
    .then(function () {
      return api.macClearConfirm({shipvoyageItemId:clearConfirmRow.shipVoyageItemId})
    })
    .then(res => {
      getList(queryParams.value)
      proxy.$modal.msgSuccess(res.msg)
