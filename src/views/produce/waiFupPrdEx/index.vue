...
import waiFupPrdExApi from '@/api/produce/waiFupPrdEx/index.js'
import moment from "moment";
import SearchHeader from "@/components/SearchHeader/index.vue";
import * as XLSX from "xlsx";
const disabledSelct = ref(false)
...
const buttonList = reactive([
  {
    ...
    type: 'primary', // 按钮类型
    click: () => prdEx, // 回调函数
    permission: 'produce:waiFuPrdEx:prdEx', // 权限

  }, {
    label: '销审', // 按钮名称
    type: 'primary', // 按钮类型
    click: () => prdExV, // 回调函数
    permission: 'produce:waiFuPrdEx:prdExV', // 权限

  },  {
    label: '导出', // 按钮名称
    type: 'primary', // 按钮类型
    click: () => exportExcel, // 回调函数
    permission: 'produce:waiFuPrdEx:exportExcel', // 权限
...
const prdEx= async ()=>{
  ...
  await waiFupPrdExApi.prdEx(queryParams.value).then(res => {
...
const prdExV= async ()=>{
  ...
  await waiFupPrdExApi.prdExV(queryParams.value).then(res => {
    tableData.value = res.data
    total.value = res.data.totalNum
    loading.value = false
  })
...
const exportExcel = async (excelName) => {
  await waiFupPrdExApi.isTrueCompany(queryParams.value).then(res=>{
    if(res.data.flag===true){
      ...
      proxy.$message.error("当前登录人只能导出当前登录人部门下的数据，您当前所属："+res.data.company)
    }
...
const init = async () => {
  let userInfo ={}
  await waiFupPrdExApi.getNowUser().then(res=>{
    userInfo = res.data
  })
  ...
  nextTick(() => {
    ...
    SearchHeaderRef.value.searchData.status = '10'
    if(userInfo.isSuperadmin!='1'){
      SearchHeaderRef.value.searchData.deptId = userInfo.deptId
      queryParams.value.deptId = userInfo.deptId
      disabledSelct.value = true
    }else{
      disabledSelct.value = false
    }
