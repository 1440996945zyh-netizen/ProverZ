index.vue<template>
  <div class="formData" style="padding: 10px 10px 10px 10px">
    <div class="formData" style="padding: 5px 0px 0px 0px;height: 50px;">
      <el-row>
        ...
        <el-col :span="3" style="float: right;">
          <el-button type="danger" @click="daoYunAudit('LZ','0')" style="float: right;margin-left: 10px">撤销</el-button>
          <el-button type="primary" @click="daoYunAudit('LZ','1')" style="float: right;margin-left: 0px">审核</el-button>
        </el-col>
      </el-row>
    </div>
    <el-table :data="tableData" :span-method="bottomSpanMethod" border style="width: 100%;margin-bottom: 8px" ref="jsdRef">
      ...
      <el-table-column prop="ddStatus" label="调度审核" width="130">
        <template #default="scope">
          <el-tag type="danger"  v-if="scope.row.ddStatus=='0'">未审核</el-tag>
          <el-tag type="success" v-if="scope.row.ddStatus=='1'">{{scope.row.dStatusByName}}已审核</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="kcStatus" label="库场审核" width="130">
        <template #default="scope">
          <el-tag type="danger"  v-if="scope.row.kcStatus=='0'">未审核</el-tag>
          <el-tag type="success"  v-if="scope.row.kcStatus=='1'">{{scope.row.kStatusByName}}已审核</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="lzStatus" label="分配审核" width="130">
        <template #default="scope">
          <el-tag type="danger"  v-if="scope.row.lzStatus=='0'">未审核</el-tag>
          <el-tag type="success" v-if="scope.row.lzStatus=='1'">{{scope.row.lStatusByName}}已审核</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="wfStatus" label="外付审核" width="130">
        <template #default="scope">
          <el-tag type="danger" v-if="scope.row.wfStatus=='0'">未审核</el-tag>
          <el-tag type="success" v-if="scope.row.wfStatus=='1'">{{scope.row.wStatusByName}}已审核</el-tag>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
const daoYunAudit=(type,value)=>{
  ...
  if(type === 'DD'){
    tableData.value.forEach(e=>{
      if(value == '1'){
        ...
        if(e.kcStatus=='0' && value == '0'){
          e.ddStatus = value
        }else{
          flag = false;
          proxy.$message.error("请先撤销库场审核");
          return;
        }
      }
    })
    ...
    tableData.value.forEach(e=>{
      if(value == '1'){
        ...
        if(e.lzStatus=='0'){
          ...
          flag = false;
          proxy.$message.error("请先撤销流机和装卸审核");
          return;
        }
      }
    })
  ...
  if(flag){
    api.updateSettlementStatus(tableData.value,queryParams.value.startDate,"lj").then(res=>{getList(queryParams.value) })
  }
}
const init = async () => {
