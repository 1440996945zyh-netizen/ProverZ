<template>
	...
	<el-drawer v-model="tallyVisible" title="理货记录" size="89%">
		...
		<BaseTable class="tab_table"
               :tableColumns="tallyTableColumns"
               :tableData="tallytableData"
               ref="baseTable"
               :tableHeight="450"
    />
		<div class="pagination-container">
      <div slot="pagination-container" class="pagination" style="display: inline">
                {{collectHtml}}
...
const constButtonList = reactive([
	{
		label: '理货确认', // 按钮名称
		type: 'primary', // 按钮类型
		click: () => getDispatchTicket, // 回调函数
...
const lgButtonList = reactive([
	{
		label: '辅助签票', // 按钮名称
		type: 'primary', // 按钮类型
		click: () => lgTicket, // 回调函数
		permission: 'produce:dispatchSecondary:fuzhuDispatch', // 权限
	},
	{
		label: '撤销', // 按钮名称
		type: 'primary', // 按钮类型
		click: () => lgDispatchRepeal, // 回调函数
		permission: 'produce:dispatchSecondary:fuzhuRepeal', // 权限
	},
])
...
const lgTicket = () => {
  detailVisible.value = true
  let cargoList = [{
    value: clickRow.value.cargoCode,
    label: clickRow.value.cargoName
  }]
  setStartEndTime(clickRow.value)
  proxy.setFormData(statusList.value, clickRow.value)
  // console.log('statusList.value', statusList.value)
  nextTick(() => {
    detailRef.value.resetTableData()
    detailRef.value.signType = 1 //签票类型
    detailRef.value.date.startTime = clickRow.value.startWorkTime
    detailRef.value.date.endTime = clickRow.value.endWorkTime
    detailRef.value.planType = queryParams.value.planType
    detailRef.value.processCode = clickRow.value.processCode //主作业过程e
    detailRef.value.cargoList = cargoList
    detailRef.value.getLgTrustCargo(clickRow.value.id, 4, clickRow.value.cargoCode, clickRow.value.processCode)
    detailRef.value.getLgProcess(clickRow.value.processCode, clickRow.value.cargoCode, clickRow.value.id)
    detailRef.value.getLgHatchData(clickRow.value.hatchNum)
    detailRef.value.setCargoList()
    detailRef.value.lgInit()
    // detailRef.value.processCode = clickRow.value.processCode //主作业过程
    // // 零工计划时可编辑列表展示字段不同
    // if (queryParams.value.planType == '4') {
    //   detailRef.value.tableColumns = detailRef.value.jobTableColumns
...
const handleMainClick = tab => {
    ...
    buttonList.push(...constButtonList)
	} else if (tab.index === '1') {
		queryParams.value.planType = '4'
    buttonList.splice(0, buttonList.length);
    buttonList.push(...lgButtonList)
	} else if (tab.index === '2') {
...
const save = async () => {
    ...
    const params = {
      ...
      soure:"PC",
      shipvoyageId:clickRow.value.shipvoyageId,
    ...
    //零工签票
          ...
          proxy.$modal.confirm('确定保存?').then(() => {
            ...
            params.ticketType = 4
            api.lgInsertWorkTicket(params).then(res => {
              if (res.code === '0000') {
                proxy.$modal.msgSuccess(res.msg)
                detailVisible.value = false
                getList(queryParams.value)
...
const gettableList = async () => {
	...
	await api.getTallyRecord(queryPage.value).then(res => {
     ...
     api.getSummaryQuantityTon(queryPage.value).then(res => {
      console.log('件数、吨数汇总', res)
       collectHtml.value = ' 总计数：'+res.data.quantity+',总吨数：'+res.data.ton
       console.log(collectHtml.value)
    })
	})
  //底部合计
