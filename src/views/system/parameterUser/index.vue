<template>
	<div class="app-container">
		<div class="selectBox">
			<div></div>
			<div>
				<el-button v-hasPermi="['system:parameterUser:query']" type="primary" icon="Search" @click="handleQuery">刷新</el-button>
				<el-button v-hasPermi="['system:parameterUser:save']" type="primary" icon="CircleCheck" @click="doSave">保存</el-button>
        <el-button v-hasPermi="['system:parameterUser:checkLog']" type="primary" icon="Check" @click="checkLog">日志</el-button>
      </div>
		</div>
		<EditTable
			ref="editTableRef"
...
<script setup name="parameterUser">
...
const parameterUserLogVis = ref(false)
const parameterUserLogDetailRef = ref(null)
			...
			return [
				h(
					ElButton,
					{
						...
						link: true,
						icon: 'Delete',
            permission: 'system:parameterUser:delete', // 权限
					},
					{
						default: () => '删除',
					}
...
/** 查询参数列表 */
const getList = e => {
	api.getUserList({flag:1}).then(response => {
...
const doSave = async () => {
	if (await editTableRef.value.validAllEvent()) {
		proxy.$modal.confirm('确定保存？').then(function () {
			api.listSaveUser(tableData.value).then(response => {
				getList()
...
const checkLog = () => {
  parameterUserLogVis.value = true
  api.checkParLog({flag:1}).then(res => {
    parameterUserLogDetailRef.value.tableData =res.data
  })
}
getList()
</script>
