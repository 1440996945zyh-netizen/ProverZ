	...
	<el-form :model="addReceiptForm" ref="ruleForm" label-width="120px" :rules="rules">
		<el-collapse v-model="activeNames">
			<el-collapse-item title="基本信息" name="order">
				...
				<el-row>
					<el-col :span="6">
						<el-form-item label="贷方票据类型" prop="cndnBillTypeCode">
							<Select
								:dataConfig="{ params: { type: 'DICT', dictType: 'CNBILLTYPE' } }"
								v-model:value="addReceiptForm.cndnBillTypeCode"
								v-model:label="addReceiptForm.cndnBillTypeName"
								placeholder="请选择贷方票据类型"
								@change="selectChange"
							/>
						</el-form-item>
					</el-col>
			...
			<el-collapse-item name="cccc" v-if="addinfoShowFlag" title="多收款发票">
				<BaseTable
					ref="editTable"
					:tableHeight="300"
...
const addReceiptForm = reactive({
	...
	invoiceCode: '', //发票号码
	cndnBillTypeCode: '1', //贷方票据类型
	cndnBillTypeName: '', //贷方票据类型
...
const edit = row => {
	...
	nextTick(() => {
		...
		detailRef.value.getRateList() // 清空事件
    proxy.setFormData(detailRef.value.formData, editRow)
    detailRef.value.formData.rateName = ''
  })
}
const edit2 = row => {
	title.value = '编辑'
