...
const changeSelect = e =>{
  ...
  console.log(e,"e")
  console.log(e.cargoOwnerId,"ecargoOwnerIdcargoOwnerId")
  //获取预缴编号
  nextTick(()=>{
    ...
    api.getPrepaymentCodeList({customerId:formData.customerId,prepaymentTypeCode:formData.prepaymentTypeCode}).then(res=>{
      prepaymentCodeList.value = res.data
    })
  })
  nextTick(()=>{
    api.getAmountInfo({busTrustId:formData.busTrustId,companyId:formData.companyId,customerId:formData.customerId,cargoInfoId:formData.cargoInfoId}).then(res=>{
      console.log(res,"res");
...
const prePayTypeModeList = ref([{label:'大预缴',value:'10'},{label:'货物预缴',value:'20'}])
...
const prepayTypeChange = () =>{
    ...
    formData.trustNo= '',
    formData.voidRemark='',
    formData.prepayModeCode='',//预缴方式
    formData.prepayModeName='',//预缴方式名称
    formData.trustLabel='',
    formData.trustId='',
    formData.prePayId ='',
    formData.prePayCode =''
  ...
  if(formData.prepaymentTypeCode!='10'){
    prePayTypeModeList.value.forEach(item=>{
      if(item.value=='20'){
        item.disabled=true
      }
    })
...
const resetForm = () => {
    ...
    formData.prepaymentTypeCode= '',
    formData.prepaymentTypeName= '',
    formData.customerName= '', //客户名称
    formData.customerId= '', //客户code
