...
const ALLOWED_TYPES = { // 变量声明统一用const（更符合ES规范）
...
function isAllowed(propName, propDescriptor, newElement) {
  const name = propDescriptor.name, // 变量声明统一用const
...
function FlowableModdleExtension(eventBus) {
  eventBus.on(
    "property.clone",
    function(context) {
      const newElement = context.newElement, // 变量声明统一用const
...
// 核心修复：用ES模块的default导出替代CommonJS的module.exports
